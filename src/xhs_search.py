...
async def get_xhs_search_keywords(context,words,item_type,sort_type,counts) -> dict[int, list,str]:
    resp_messages = {
        "code":200,
        "data":[],
        "message":"",
    ...
    await page.wait_for_timeout(5000)
    await page.locator(".filter-panel").get_by_text("收起").click()
    # 滚动鼠标向下30次
...
async def get_article_info(context,id,xsec_token)->dict:
